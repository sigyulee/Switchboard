...
python3 Tests/DriverChecks/BuildMetadataChecks.py
python3 Tests/DriverChecks/BuildIdentityChecks.py
python3 Tests/DriverChecks/PackageSanitizationChecks.py
...
xcrun swiftc -swift-version 6 -D INSTALLER_TESTING -parse-as-library \
    -target arm64-apple-macosx26.0 -module-cache-path "$check_directory/module-cache" \
    Installer/Installer.swift Tests/DriverChecks/InstallerChecks.swift \
    -o "$check_directory/InstallerChecks"
"$check_directory/InstallerChecks"

for name in MIHCaller MIHReply SwitchboardAgent; do
