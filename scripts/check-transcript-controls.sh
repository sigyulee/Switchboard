...
check_directory="$(mktemp -d "${TMPDIR:-/tmp}/switchboard-transcript-controls-check.XXXXXX")"
trap 'rm -rf "$check_directory"' EXIT

# Reuse the modules already built by BridgeChecks in check.sh; never race another SwiftPM build.
# AppModel runs in preview with temporary packages and volatile, isolated defaults.
...
executable = temporary / 'ControlsChecks'
subprocess.run([
    'xcrun', 'swiftc', '-swift-version', '6', '-parse-as-library',
    '-target', 'arm64-apple-macosx26.0', '-module-cache-path', '.build/transcript-controls-clang-cache',
    '-I', str(binary / 'Modules'), '-I', str(binary / 'AudioRealtime.build'),
    *[f'Sources/Switchboard/{name}.swift' for name in sources],
    'Tests/TranscriptControlsChecks/ControlsChecks.swift', *map(str, audio_objects), *objects,
    '-o', str(executable),
], check=True)
