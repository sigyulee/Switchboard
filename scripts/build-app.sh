...
xcrun swiftc -swift-version 6 -O -target arm64-apple-macosx27.0 \
...
cp -R build/drivers/MIHCaller.driver build/drivers/MIHReply.driver "$app/Contents/Resources/Drivers/"
...
p={'CFBundleIdentifier':'com.switchboard.main','CFBundleExecutable':'Switchboard',
   ...
   'CFBundleShortVersionString':sys.argv[3],'CFBundleVersion':sys.argv[3],'LSMinimumSystemVersion':'27.0',
   ...
   'NSMicrophoneUsageDescription':'Switchboard receives Phone audio from a virtual device to send to the Agent, monitor, and record.',
   'NSAudioCaptureUsageDescription':'Switchboard sends Agent audio playing in Chrome to the call and records it.',
...
hashes={str(f.relative_to(drivers)):hashlib.sha256(f.read_bytes()).hexdigest()
        for b in ['MIHCaller.driver','MIHReply.driver'] for f in (drivers/b).rglob('*') if f.is_file()}
