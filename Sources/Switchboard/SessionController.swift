...
@MainActor @Observable final class SessionController {
    ...
    var running: Bool { state?.lifecycle == .running }
    var paused: Bool { state?.lifecycle == .paused }
    var active: Bool { state != nil && !closed }
    var recording: Bool { state?.effectiveAudioRecording == true }
    ...
    func updateElapsed(_ seconds: Double) {
        ...
        do {
            try next.advance(to: max(next.durationFrames, Int64(seconds * 48_000)))
            state = next
