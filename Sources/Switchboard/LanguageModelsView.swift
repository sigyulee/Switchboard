...
struct LanguageModelsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack {
                Text(strings(.transcriptModels)).font(typography.panelTitle)
                Spacer()
            ...
            ForEach(AudioSide.allCases, id: \.self) { side in
                ...
                VStack(alignment: .leading, spacing: 10) {
                    Text((side == .caller ? strings(.roleCaller) : "Agent") + " · " + name).font(
                        typography.section)
            ...
            if let error { Text(error).foregroundStyle(.orange).font(typography.caption) }
