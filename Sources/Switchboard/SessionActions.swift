...
extension AppModel {
    func selectApplication(_ application: ApplicationIdentity, forAgent: Bool) {
        ...
        do {
            ...
            profile.callerDevicesConfirmed = false
    ...
    func confirmCallerDevices(_ confirmed: Bool) {
        guard !session.active, var profile = routeProfile else { return }
        profile.callerDevicesConfirmed = confirmed
        routeProfile = profile
        do { if !preview { defaults.set(try JSONEncoder().encode(profile), forKey: "routeProfile") } } catch {
...
extension AppModel {
    func setTranscriptionEnabled(_ enabled: Bool) {
        ...
        if enabled {
            transcript.panelVisible = true
            if transcript.canStart {
                transcript.start(session: session)
            } else {
                transcript.showConfiguration = true
            }
        } else {
            Task { await transcript.stop(session: session) }
        }
    }
}
