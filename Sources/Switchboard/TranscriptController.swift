...
@MainActor @Observable final class TranscriptController {
    ...
    private(set) var changingConfiguration = false
    ...
    var isProcessing: Bool {
        catchingUp || (activeToken != nil && states.values.contains { $0.speech == .ready })
    }
    ...
    private var configurationAction = ConfigurationAction.configure

    enum LanguageSetting { case caller, agent, target }
    private enum ConfigurationAction { case configure, recordOnly, restart }
    ...
    var canStart: Bool {
        readiness.canStart(
            configuration: configuration, busy: busy || changingConfiguration, stopping: stopOperation != nil)
    }
    var needsSpeechDownload: Bool {
        states.values.contains { $0.speech == .downloadRequired }
            || readiness.installedStates.values.contains { $0.speech == .downloadRequired }
    }

    func needsTranslationDownload(for side: AudioSide) -> Bool {
        states[side]?.translation == .downloadRequired
            || readiness.installedStates[side]?.translation == .downloadRequired
    }
    ...
    func setLanguage(_ value: String, for setting: LanguageSetting, session: SessionController) {
        switch setting {
        case .caller:
            guard callerLanguage != value else { return }
            callerLanguage = value
        case .agent:
            guard agentLanguage != value else { return }
            agentLanguage = value
        case .target:
        ...
        error = nil
        readiness = TranscriptReadinessState()
        requestConfigurationChange(session: session, action: .configure)
    }

    func recordOnly(session: SessionController) {
        requestConfigurationChange(session: session, action: .recordOnly)
    }

    func disable(session: SessionController) {
        requestConfigurationChange(session: session, action: .configure)
    }

    private func requestConfigurationChange(session: SessionController, action: ConfigurationAction) {
        configurationAction = action
        showConfiguration = true
        panelVisible = action != .recordOnly
        capabilityTask?.cancel()
        // Revoke accepted startup before scheduling the stop; a later picker choice joins the same stop.
        startupGate.invalidate()
        task?.cancel()
        guard configurationTask == nil else { return }
        guard session.active || activeToken != nil || task != nil else {
            refreshCapabilities()
            return
        }
        ...
        configurationTask = Task { [weak self] in
            ...
            guard !Task.isCancelled, session.sessionID == sessionID else { return }
            states = readiness.installedStates
            refreshCapabilities()
            if configurationAction == .restart { start(session: session) }
        }
    }

    func finishConfigurationChanges() async {
        await configurationTask?.value
    }

    func translationPreparationFailed(_ message: String, configuration: TranscriptConfiguration) {
        guard self.configuration == configuration else { return }
        capabilityTask?.cancel()
        for side in AudioSide.allCases where !configuration.skipsTranslation(for: side) {
            var state =
                states[side] ?? TranscriptSideState(side: side, speech: .stopped, translation: .failed)
    ...
    func downloadSpeech() {
        guard !busy, !changingConfiguration, let configuration else { return }
        busy = true
        capabilityTask?.cancel()
        task = Task { [weak self] in
            ...
            do {
                ...
                for side in AudioSide.allCases {
                    guard var state = states[side],
                        [.downloadRequired, .failed, .resourceLimit].contains(state.speech)
                    else { continue }
                    state.speech = .failed
                    state.message = String(strings.error(error).prefix(2048))
                    states[side] = state
                }
            }
        }
    ...
    func start(session: SessionController) {
        guard !changingConfiguration else { return }
        if activeToken != nil {
            requestConfigurationChange(session: session, action: .restart)
            return
        }
    ...
    func finishForSave(session: SessionController) async {
        ...
        await finishConfigurationChanges()
    }
    ...
    private func receiveTranslationStatus(_ entry: TranscriptEntry) {
        guard var state = states[entry.side] else { return }
        guard let translation = TranscriptEnginePresentation.translationState(for: entry.translationStatus)
        else {
            return
        }
    ...
    private func receive(_ event: TranscriptEvent, operation: UUID) {
        ...
        case .partial(let entry), .final(let entry), .upsert(let entry):
            receiveTranslationStatus(entry)
...
struct TranscriptEnginePresentation {
    enum Kind { case speech, translation }
    let state: TranscriptModelState?
    let issue: TextKey?
    let details: String?

    init(kind: Kind, states: [AudioSide: TranscriptSideState]) {
        let ordered = AudioSide.allCases.compactMap { states[$0] }
        let modelStates = ordered.map { kind == .speech ? $0.speech : $0.translation }
        state = [.failed, .resourceLimit, .unsupported, .downloadRequired, .ready, .stopped, .notNeeded]
            .first { modelStates.contains($0) }
        let failed = state == .failed || state == .resourceLimit
        issue = failed ? (kind == .speech ? .transcriptSpeechFailure : .transcriptTranslationFailure) : nil
        let messages = ordered.filter {
            let value = kind == .speech ? $0.speech : $0.translation
            return value == .failed || value == .resourceLimit
        }.compactMap(\.message).filter { !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
        var seen: Set<String> = []
        let text = messages.filter { seen.insert($0).inserted }.joined(separator: "\n")
        details = text.isEmpty ? nil : String(text.prefix(4096))
    }

    static func translationState(for status: TranscriptTranslationStatus) -> TranscriptModelState? {
        switch status {
        case .failed: .failed
        case .dropped: .resourceLimit
        case .downloadRequired: .downloadRequired
        case .unsupported: .unsupported
        case .translated: .ready
        default: nil
        }
    }
}
