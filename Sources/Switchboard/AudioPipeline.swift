...
final class AudioPipeline: @unchecked Sendable {
    ...
    private var sessionOrigin: UInt64 = 0
    ...
    private func applyConfiguration() {
        ...
        if let target = next.agentTarget, tap == nil {
            do {
                ...
                tap = newTap
                state.agentError = nil
            } catch { state.agentError = AppFailure(error) }
        }
        if relaying {
            configureOutput(&agentInput, id: next.agentInputID, failure: &state.callerError)
            configureOutput(&reply, id: next.replyID, failure: &state.agentError)
            ...
            // Destroying each output stops its callback before freeing the queue.
            // No samples queued before Pause can reappear after Resume.
            agentInput = nil
            reply = nil
            monitor = nil
        }
        state.callerReady = caller != nil && (!relaying || agentInput != nil)
        state.agentReady = agent != nil && (!relaying || reply != nil)
    ...
    func startSession(directory: URL, manifest: RecordingManifest) async throws {
        ...
        await work.complete { [self] in
            ...
            sessionID = id
            sessionOrigin = mach_continuous_time()
    ...
    private func elapsedFrame() -> Int64 {
        ...
        let elapsed = sb_host_seconds(mach_continuous_time() - sessionOrigin) * PCM.rate
        guard elapsed.isFinite, elapsed < Double(Int64.max) else { return Int64.max - 1 }
        return Int64(max(0, elapsed))
    }
    ...
    private func tick() {
        ...
        consume(caller, side: .caller, now: now, sessionFrame: frame)
        consume(agent, side: .agent, now: now, sessionFrame: frame)
    ...
    private func consume(_ endpoint: AudioEndpoint?, side: AudioSide, now: Double, sessionFrame: Int64) {
        guard let endpoint else { return }
        do {
            for packet in try endpoint.packets() {
                guard relaying, packet.seconds >= acceptingAfter else { continue }
                ...
                if side == .caller {
                    ...
                    agentWaveform.insert(peak: level, seconds: packet.seconds)
                    agentTimeline.insert(packet.samples, at: start)
                    state.agentLevel = max(state.agentLevel, level)
                    try reply?.push(packet.samples)
                }
                let frame = sessionFrame - Int64(((now - packet.seconds) * PCM.rate).rounded())
