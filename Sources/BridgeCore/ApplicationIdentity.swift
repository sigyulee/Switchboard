...
public struct RouteProfile: Codable, Equatable, Sendable {
    ...
    public var callerDevicesConfirmed: Bool

    public init(
        agent: ApplicationIdentity, caller: ApplicationIdentity, callerDevicesConfirmed: Bool = false
    ) throws {
        self.agent = agent
        self.caller = caller
        self.callerDevicesConfirmed = callerDevicesConfirmed
        try validate()
    }
