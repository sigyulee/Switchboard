...
public struct SessionState: Codable, Equatable, Identifiable, Sendable {
    ...
    /// Add session metadata to a closed legacy recording without replaying lifecycle actions.
    /// Its audio manifest remains authoritative; unknown control history stays empty.
    public init(archiving recording: RecordingManifest) throws {
        try recording.validate()
        guard recording.status != .recording, recording.status != .finalizing else {
    ...
    /// Change only the displayed name, preserving the complete session timeline and state.
    public func renamed(to name: String) throws -> Self {
        var renamed = self
